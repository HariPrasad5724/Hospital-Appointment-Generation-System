...
void allotWaitList(struct Visitor v[],int idx)
{
	...
	while(1)
	{
		x=w[front];
     	front = front + 1;
     	cout<<"Are you interested to give this slot to patient with token number "<<x.Token<<"\n\tIf yes enter 1 or enter 2\nEnter your choice : ";
     	cin>>c;
    	if(c==1)
     	{
			v[idx]=x;
...
void  SearchToken(struct Visitor V[])
{
        int token;
        cout<<"\nTOKEN NO. ";
        cin>>token;
        int index=Hashcode(token);
        if(V[index].Age!=-1)
        {
                cout<<"\n\nNAME:        "<<V[index].Name;
                cout<<"\nPHONE NUMBER:"<<V[index].Phnno;
                cout<<"\nAGE:         "<<V[index].Age;
        }
        else
        {
